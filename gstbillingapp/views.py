...
from .models import Product, Department, Item_description, ARC_number, Item_number, SAC_number, Service_description, UOM, Service_number
...
def invoice_create(request):
    ...
    context['default_invoice_date'] = datetime.strftime(datetime.now(), '%Y-%m-%d')
    context['department'] = Department.objects.all()
    context['item_description'] = Item_description.objects.all()
    context['arc_number'] = ARC_number.objects.all()
    context['item_number'] = Item_number.objects.all()
    context['sac_number'] = SAC_number.objects.all()
    context['service_description'] = Service_description.objects.all()
    context['service_number'] = Service_number.objects.all()
    ...
    if request.method == 'POST':
        ...
        new_invoice = Invoice(user=request.user,
                              invoice_number="KC/" + invoice_data['invoice-number'] + "/" + str(datetime.now().year),
...
def invoice_viewer(request, invoice_id):
    ...
    context['user_profile'] = user_profile
    return render(request, 'gstbillingapp/invoice_print.html', context)
