...
def invoice_data_processor(invoice_post_data):
    ...
    processed_invoice_data['item_number'] = invoice_post_data['item-number']
    ...
    processed_invoice_data['arc_number'] = invoice_post_data['arc-number']    
    ...
    for idx, item in enumerate(invoice_post_data['invoice-item']):
        if item:
            print(idx, item)
            item_entry = {}
            item_entry['invoice_item'] = item
            item_entry['invoice_sac'] = invoice_post_data['invoice-sac'][idx]
...
def update_products_from_invoice(invoice_data_processed, request):
    for item in invoice_data_processed['items']:
        ...
        if Product.objects.filter(user=request.user,
                                  product_description=item['invoice_description'],
                                  product_sac=item['invoice_sac'],
