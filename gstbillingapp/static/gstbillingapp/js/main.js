...
function update_amounts(element){
    ...
    var rate_without_gst = rate_with_gst;
    var amt_without_gst = rate_with_gst * qty;
...
function product_result_to_domstr(result) {
    ...
    "<div>"+ result['product_sac'] + " | " + result['product_unit'] + " | " + result['product_gst_percentage'] +
    "</div>";
     return domstr;
}
...
function product_result_click() {
    ...
    selected_item_input.val(product_data_json['product_description']);
    selected_item_input.parent().parent().find('input[name=invoice-sac]').val(product_data_json['product_sac']);    
